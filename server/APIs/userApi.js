...
userApp.put('/comment/:articleId',expressAsyncHandler(async(req,res)=>{
    ...
    const articleWithComments = await Article.findOneAndUpdate({articleId:req.params.articleId}, //condtion
        {$push:{comments:commentObj}}, //pushed to comments array
        {returnOriginal:false}) //returns latest doc
        res.status(200).send({message:"added comment",payload:articleWithComments})
}))

//delete comment
userApp.put('/comments/:commentId', expressAsyncHandler(async (req, res) => {
